...
function ScheduleView({ data, loading }) {
  if (loading) return <div style={{ padding: 20, color: "#9896B8" }}>Loading schedule...</div>;
  const todaysAppts = data?.todaysAppointments || [];

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <CardBox>
        <WeekCalendarGrid appointments={data?.weekAppointments || []} />
      </CardBox>
      <CardBox>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {patients.map((p, i) => (
              <div key={p.id} style={{
                ...
                }}>{p.firstName?.charAt(0)}{p.lastName?.charAt(0)}</div>
                <div style={{ flex: 1 }}>
                ...
                {p.email && <div style={{ fontSize: 11, color: "#9896B8" }}>{p.email}</div>}
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {completedAppts.slice(0, 20).map(a => (
              <div key={a.id} style={{ padding: "10px 14px", borderRadius: 8, border: "1px solid #E5E1F8", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <span style={{ fontWeight: 700, color: "#1E1B4B", fontSize: 14 }}>{a.patient.firstName} {a.patient.lastName}</span>
              ...
              <div key={n.id} style={{ padding: "12px 14px", borderRadius: 8, border: "1px solid #E5E1F8", background: "#F5F3FF" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ fontWeight: 700, color: "#1E1B4B", fontSize: 13 }}>
                    {n.appointment?.patient?.firstName} {n.appointment?.patient?.lastName}
                  </span>
...
function ChatView({ data, loading }) {
  if (loading) return <div style={{ padding: 20, color: "#9896B8" }}>Loading...</div>;
  const patients = data?.patients || [];
      ...
      <CardBox>
        ...
        {!recentPatient ? (
          ...
          <div style={{ background: "#EDE9FE", borderRadius: 8, padding: 16 }}>
            ...
            <input type="text" placeholder="Type a message..." style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid #E5E1F8", marginBottom: 12, fontSize: 14, boxSizing: "border-box" }} />
            <div style={{ display: "flex", gap: 8 }}>
              <button style={{ background: "#7C3AED", color: "#fff", border: "none", borderRadius: 8, padding: "10px 16px", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>Send Feedback</button>
              <button style={{ background: "#7C3AED", color: "#fff", border: "none", borderRadius: 8, padding: "10px 16px", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>Send Message</button>
            </div>
          </div>
        )}
        {patients.length > 1 && (
          <div style={{ marginTop: 16 }}>
            ...
            {patients.slice(1).map(p => (
              <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "1px solid #E5E1F8" }}>
                ...
                <span style={{ fontWeight: 600, color: "#4C4682", fontSize: 14 }}>{p.firstName} {p.lastName}</span>
...
export default function PsychologistDashboard() {
  ...
  const views = {
    schedule: () => <ScheduleView data={dashData} loading={loading} />,
      ...
      <div style={{
        ...
        <div>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "#1E1B4B", margin: 0 }}>
            Welcome back, {dashData?.name || "Doctor"}
          </h1>
          <div style={{ fontSize: 13, color: "#4C4682", fontWeight: 600, marginTop: 4 }}>
            Specialization: <span style={{ color: "#7C3AED" }}>{dashData?.specialization || "Mental Wellness"}</span>
          </div>
        </div>
